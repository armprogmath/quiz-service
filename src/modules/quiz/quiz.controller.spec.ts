...
describe('QuizController', () => {
    ...
    it('should add a question', async () => {
        ...
        const result = await controller.addQuestion(1, dto);
    ...
    it('should start a quiz attempt', async () => {
        ...
        const req = { user: { id: 7 } };
        const result = await controller.start(1, req);
    ...
    it('should submit quiz answers', async () => {
        ...
        const result = await controller.submit(1, body, req);

        expect(result).toEqual({ score: 80 });
        expect(mockQuizService.submit).toHaveBeenCalledWith(1, 55, 7, [1, 2, 3]);
    });
});